document.addEventListener('DOMContentLoaded', () => {
    ...
    const generatedPasswordDisplay = document.getElementById('generatedPassword'); // Новый элемент для отображения пароля
    ...
    if (passwordChoice) {
        passwordChoice.addEventListener('change', (event) => {
            if (event.target.value === 'manual') {
                ...
                generatedPasswordDisplay.textContent = ''; // Скрываем отображение сгенерированного пароля
            } else {
                manualPasswordFields.style.display = 'none';
                const generatedPassword = generatePassword();
                generatedPasswordDisplay.textContent = 'Generated password: ' + generatedPassword; // Отображаем сгенерированный пароль
            }
        });
    }
    ...
    if (registerForm) {
        registerForm.addEventListener('submit', (event) => {
            ...
            const password = passwordChoice.value === 'manual' ? document.getElementById('password').value : generatePassword();
            ...
            if (passwordChoice.value !== 'manual') {
                generatedPasswordDisplay.textContent = 'Your generated password is: ' + password; // Отображаем сгенерированный пароль
            }
        });
    }
